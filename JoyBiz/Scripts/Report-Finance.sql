...
-- TRANSACTION HEADER
select 
		1 as "tax_no", t.id, t.id, 
		'FK' as "tax_type", '''' || '01' as "KD_JENIS_TRANSAKSI", '0' as "FG_PENGGANTI", '''' || '00000' as "NOMOR_FAKTUR",
		to_char(t.transaction_date, 'MM') as "MASA_PAJAK", to_char(t.transaction_date, 'YYYY') as "TAHUN_PAJAK", 
		to_char(t.transaction_date, 'MM/DD/YYYY') as "TANGGAL_FAKTUR", 
--		to_char(t.created_at, 'MM/DD/YYYY') as "TANGGAL_FAKTUR", 
		case when u.no_npwp is null then '000000000000000' else u.no_npwp end as "NPWP", 
		u.nama as "NAMA",
		ap.provinsi as "ALAMAT_LENGKAP", 
--		t.total_price as "JUMLAH_DPP", 
		(select sum(round((b.harga_1 / 1.11) * td.qty , 0)) 
		 from transaction_detail td 
		 	  inner join barang b on td.id_barang_fk = b.id
		 where td.id_trans_fk = t.id
		...
		(select round(sum(round((b.harga_1 / 1.11) * td.qty , 0)) * 0.11, 0)
		 from transaction_detail td 
		 	  inner join barang b on td.id_barang_fk = b.id
		 where td.id_trans_fk = t.id
		 		and b.kode not in ('MK001L','MK001M','MC002L','MC002XL','MC002S','MC002XXL','MK002M','MC006jp','MC006jps','MC006core','MT0002','MK001Lf','MK001XLf','MK004',
		...
		0 as "JUMLAH_PPNBM", 0 as "ID_KETERANGAN_TAMBAHAN", 0 as "FG_UANG_MUKA", 0 as "UANG_MUKA_DPP", 0 as "UANG_MUKA_PPN",
		0 as "UANG_MUKA_PPNBM", '''' || u.no_ktp as "REFERENSI", t.code_trans as "KODE_DOKUMEN_PENDUKUNG"
from "transaction" t  
...
where t.status in('PC', 'S', 'A', 'I')
	 and to_char(t.transaction_date, 'YYYY-MM-DD') between '2024-02-01' and '2024-02-20'
...
-- NPWP JOYBIZ
select 2 as "tax_no", '' as "id", '' as "id", 'FAPR' as "LT", 'PT JOY BUSINESS INTERNATIONAL' as "NPWP", 
		'BUSINESS PARK KEBON JERUK A18, JL. RAYA MERUYA ILIR KAV.88, MERUYA UTARA, KEMBANGAN, JAKARTA BARAT, DKI JAKARTA' as "NAMA", 
		'' as "JALAN", '' as "BLOK", '' as "NOMOR", '' as "RT", '' as "RW", '' as "KECAMATAN", '' as "KELURAHAN", 
		'' as "KABUPATEN", '' as "PROPINSI", '' as "KODE_POS", '' as "NOMOR_TELEPON";


-- TRANSACTION DETAIL
select 3 as "tax_no", td.id_trans_fk, td.id, 'OF' as "OF", 
		--td.id_barang_fk, b.id, 
		b.kode as "KODE_OBJEK", b.nama as "NAMA",
