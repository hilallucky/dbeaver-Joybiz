...
where 
	...
	and to_char(t.transaction_date, 'YYYY-MM') = '2024-02' 
...
-- =============================================================================================================================
-- EDIT STEP 3
select  * from barang b where b.kode in ('P001B','P001','B001B') or id in (1638);
...
--		bp.id, bp.harga_1, bp.harga_1 * (t.total_barang * bd.qty), bdx.harga_1, bdx.harga_1 * (t.total_barang * bd.qty),
--		(bp.harga_1 * (t.total_barang * bd.qty)) / (bdx.harga_1 * (t.total_barang * bd.qty))
from barang_detail bd
	 join (
			select td.id_barang_fk, td."name", sum(td.qty) as total_barang
			from "transaction" t
